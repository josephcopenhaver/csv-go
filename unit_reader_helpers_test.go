...
func testUnitReaderHelperPaths_decodeMBControlRune(t *testing.T) {
	...
	tcs := []TC{
		...
		{"0xF09FA684 - Unicorn emoji", []byte{0xF0, 0x9F, 0xA6, 0x84}, Resp{([]rune(string([]byte{0xF0, 0x9F, 0xA6, 0x84})))[0], 4}},
		...
		{"0xF09FA68410 - Unicorn emoji followed by a", []byte{0xF0, 0x9F, 0xA6, 0x84, 0x10}, Resp{([]rune(string([]byte{0xF0, 0x9F, 0xA6, 0x84})))[0], 4}},
		// incomplete number of bytes
		{"0xC2A210 - Cent Sign missing last byte", []byte{0xC2}, Resp{utf8.RuneError, 1}},
		{"0xE282AC10 - Euro sign missing last byte", []byte{0xE2, 0x82}, Resp{utf8.RuneError, 1}},
		{"0xF09FA68410 - Unicorn emoji missing last byte", []byte{0xF0, 0x9F, 0xA6}, Resp{utf8.RuneError, 1}},
		// empty
...
func testUnitReaderHelperPaths_decodeMBControlRuneWithCheckBounds(t *testing.T) {
	...
	tcs := []TC{
		...
		{"0xF09FA684 - Unicorn emoji", []byte{0xF0, 0x9F, 0xA6, 0x84}, Resp{r: ([]rune(string([]byte{0xF0, 0x9F, 0xA6, 0x84})))[0], s: 4}},
		// more than one rune worth of bytes
		...
		{"0xF09FA68410 - Unicorn emoji followed by a", []byte{0xF0, 0x9F, 0xA6, 0x84, 0x10}, Resp{r: ([]rune(string([]byte{0xF0, 0x9F, 0xA6, 0x84})))[0], s: 4}},
		// incomplete number of bytes
		{"0xC2A210 - Cent Sign missing last byte", []byte{0xC2}, Resp{p: true}},
		{"0xE282AC10 - Euro sign missing last byte", []byte{0xE2, 0x82}, Resp{p: true}},
		{"0xF09FA68410 - Unicorn emoji missing last byte", []byte{0xF0, 0x9F, 0xA6}, Resp{p: true}},
		// empty
		{"nil", nil, Resp{p: true}},
